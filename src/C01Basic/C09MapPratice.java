...
import java.util.*;
/*
Map<래퍼클래스,래퍼클래스> map = new HashMap<>();
map.put(key,value) 데이터 삽입
map.get(key) 데이터 가져오기
map.remove(key) 데이터 삭제
map.putIfAbsent(key,value) -> 데이터가 없으면 put 하겟다
map.keySet() 키 목록을 반환 , S 대문자 주의
map.values() value 목록을 set 형태로 반환
...
//            위 코드를 간결하게 한줄로 할수있다.
            System.out.println("getOrDefault 출력");
